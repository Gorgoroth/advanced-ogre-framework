...
bool GameState::keyPressed(const OIS::KeyEvent &keyEventRef)
{
    if(m_bSettingsMode == true)
    {
        ...
        {
            OgreBites::SelectMenu* pMenu = (OgreBites::SelectMenu*)AdvancedOgreFramework::getSingletonPtr()->m_pTrayMgr->getWidget("ChatModeSelMenu");
    ...
    if(AdvancedOgreFramework::getSingletonPtr()->m_pKeyboard->isKeyDown(OIS::KC_TAB))
    {
...
void GameState::onLeftPressed(const OIS::MouseEvent &evt)
{
    ...
    Ogre::Ray mouseRay = m_pCamera->getCameraToViewportRay(AdvancedOgreFramework::getSingletonPtr()->m_pMouse->getMouseState().X.abs / float(evt.state.width),
        AdvancedOgreFramework::getSingletonPtr()->m_pMouse->getMouseState().Y.abs / float(evt.state.height));
    ...
    for(itr = result.begin(); itr != result.end(); itr++)
    {
        if(itr->movable)
        {
            AdvancedOgreFramework::getSingletonPtr()->m_pLog->logMessage("MovableName: " + itr->movable->getName());
            m_pCurrentObject = m_pSceneMgr->getEntity(itr->movable->getName())->getParentSceneNode();
            AdvancedOgreFramework::getSingletonPtr()->m_pLog->logMessage("ObjName " + m_pCurrentObject->getName());
...
void GameState::update(double timeSinceLastFrame)
{
    ...
    if(!AdvancedOgreFramework::getSingletonPtr()->m_pTrayMgr->isDialogVisible())
    {
...
void GameState::buildGUI()
{
    ...
    m_pDetailsPanel = AdvancedOgreFramework::getSingletonPtr()->m_pTrayMgr->createParamsPanel(OgreBites::TL_TOPLEFT, "DetailsPanel", 200, items);
    m_pDetailsPanel->show();

    Ogre::String infoText = "[TAB] - Switch input mode\n\n[W] - Forward / Mode up\n[S] - Backwards/ Mode down\n[A] - Left\n";
    infoText.append("[D] - Right\n\nPress [SHIFT] to move faster\n\n[O] - Toggle FPS / logo\n");
    infoText.append("[Print] - Take screenshot\n\n[ESC] - Exit");
    AdvancedOgreFramework::getSingletonPtr()->m_pTrayMgr->createTextBox(OgreBites::TL_RIGHT, "InfoPanel", infoText, 300, 220);

    Ogre::StringVector chatModes;
    chatModes.push_back("Solid mode");
    chatModes.push_back("Wireframe mode");
    chatModes.push_back("Point mode");
    AdvancedOgreFramework::getSingletonPtr()->m_pTrayMgr->createLongSelectMenu(OgreBites::TL_TOPRIGHT, "ChatModeSelMenu", "ChatMode", 200, 3, chatModes);
}
